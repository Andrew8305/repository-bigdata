...
public class ESBulkWithIDWriter extends Writer {
	...
	private Logger logger = Logger.getLogger(ESBulkWithIDWriter.class.getCanonicalName());
	...
	private void bulkSaveOrUpdate(List<ESEntity> pojos, String database, String table) {
		...
		for (ESEntity pojo : pojos) {
			IndexRequestBuilder irb = client.prepareIndex().setIndex(database).setType(table).setId(pojo.get_id());
			pojo.set_id(null);
			irb.setSource(gson.toJson(pojo));
			prepareBulk.add(irb);
		}
